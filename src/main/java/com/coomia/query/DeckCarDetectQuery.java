 ...
 * 不同车身、不同车型、不同品牌、不同区域(deviceID)同时出现(within time interval) 
 *  Date: 2020年9月15日
 * 上午9:02:49
 * 
...
public class DeckCarDetectQuery {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Script intervalMap =new Script("for(int i=0; i< doc['location'].length; i++) { doc['location'][i].arcDistance(\"doc['bayonetLatitude'][i]\"+\",\"+\"doc['bayonetLongitude'][i]\")}");
    ...
    Script script =
        new Script(ScriptType.INLINE, "expression", "plateColorDescDistinct >havingCount || vehicleClassDescDistinct >havingCount || VehicleBrandDistinct > havingCount", havingScriptParam);
    BucketSelectorPipelineAggregationBuilder having = PipelineAggregatorBuilders
    ...
    ssb.aggregation(AggregationBuilders.terms("GroupbyPlateNo").field("PlateNo")
        .subAggregation(color).subAggregation(clas).subAggregation(brand).subAggregation(interval)
    ...
    for (org.elasticsearch.search.aggregations.bucket.terms.Terms.Bucket bks : data) {
      ...
      System.out
          .println("PlateNo: " + bks.getKey() + " Count: " + bks.getDocCount() + " PlateNo: ");
    }
